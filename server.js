...
/**
 * GET /api/buckets?days=5&device=INPUT
 * Returns 10-minute aggregated pulse counts, optionally filtered by device
 */
app.get('/api/buckets', (req, res) => {
  ...
  const sql = device
    ? `SELECT (server_time/600000)*600000 AS bucket_ms, COUNT(*) AS count,
              MIN(server_time) AS first_pulse_ms, MAX(server_time) AS last_pulse_ms
       FROM pulses WHERE server_time >= ? AND device_id = ?
  ...
  const params = device ? [since, device] : [since];
...
/**
 * GET /api/stats?device=INPUT
 * Summary statistics, optionally filtered by device
 */
app.get('/api/stats', (req, res) => {
  ...
  const sql = device
    ? `SELECT COUNT(*) AS total_pulses, MIN(server_time) AS first_ms, MAX(server_time) AS last_ms
       FROM pulses WHERE server_time >= ? AND device_id = ?`
    : `SELECT COUNT(*) AS total_pulses, MIN(server_time) AS first_ms, MAX(server_time) AS last_ms
       FROM pulses WHERE server_time >= ?`;
